...
const DashboardPageView = ({ trans }) => {
    ...
    const fetchEnrollmentBarStats = async (selectedYear) => {
      ...
      try {
        const response = await axiosInstance.get(
          `/api/dashboard/enrollmentBarStats?year=${selectedYear}&type=manager`
        );
        console.log("gdfgfdgf", response);
  ...
  const course_dynamicSeries = pieStats?.topCourses?.map(
    (course) => course?.average_rating
  );
  const course_dynamicTitles = pieStats?.topCourses?.map(
    (course) => course?.title
  );

  const category_dynamicSeries = pieStats?.topCategories?.map((course) =>
    parseInt(course?.courses_count, 10)
  );
  const category_dynamicTitles = pieStats?.topCategories?.map(
  ...
  const getCourses = async () => {
    try {
      const { data } = await axiosInstance({
        url: `api/users/${user.id}?populate[course_enrollments][populate][course][populate][instructors]=true&status=published`,
        method: "get",
      });
            ...
            <div className="grid grid-cols-12 gap-6">
              <div className="col-span-12 lg:col-span-12">
                <Card>
                  <CardContent>
                    <GroupedBar
                      dynamicSeries={enrollmentBarStats}
                ...
                <Card>
                  ...
                  <CardContent>
                    {course_dynamicSeries?.length > 0 ? (
                      <GradiantDonut
                        dynamicSeries={course_dynamicSeries}
                        dynamicTitles={course_dynamicTitles}
                      />
                    ) : (
                ...
                <Card>
                  ...
                  <CardContent>
                    {category_dynamicSeries?.length > 0 ? (
                      <GradiantDonut
                        dynamicSeries={category_dynamicSeries}
                        dynamicTitles={category_dynamicTitles}
                      />
                    ) : (
                      <p className="text-muted-foreground text-center text-lg">
                        No Activity Found
