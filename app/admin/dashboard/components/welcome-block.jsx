...
const WelcomeBlock = () => {
  ...
  const getCurrentUserRating = async () => {
    try {
      ...
      const { data } = await axiosInstance({
        url: `/api/points?populate[user][populate][profileImage][fields][0]=url`,
        method: "GET",
      });
      console.log("getCurrentUserRating", data.data);
      // Sort the data by `point` in descending order
      const sortedData = data.data.sort((a, b) => b.point - a.point);

      // Find the current user's rating and rank
      const currentUserRating = sortedData.find((item) => item.user.id === userId);
      
      // Calculate the current user's rank
      const rank = sortedData.findIndex((item) => item.user.id === userId) + 1;

       // Update the state with both Rank and Points in a single object array
       setUserData([
        { title: "Rank", total: rank },   // Set rank
        { title: "Points", total: currentUserRating ? currentUserRating.point : 0 }, // Set points
      ]);
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div className="w-full h-full bg-primary rounded-md  flex p-6 relative " >
      ...
      <div className="absolute top-4  mobileAvatar  sm:hidden lg:hidden ltr:right-4 rtl:left-4 ltr:md:right-[30%] rtl:md:left-[30%] ltr:md:bottom-5 ltr:2xl:right-10 rtl:2xl:left-10 w-[100px] ">
