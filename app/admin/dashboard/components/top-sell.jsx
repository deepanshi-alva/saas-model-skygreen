...
const TopSell = () => {
  ...
  useEffect(() => {
    const fetchEnrollments = async () => {
      try {
        const response = await axiosInstance.get("/api/dashboard/newEnrollments?type=admin");
      ...
      {enrollments.length > 0 ? (
        enrollments.map((item, index) => (
          ...
          >
            ...
            <span className="text-sm font-normal text-default-800 ml-4 truncate">{item?.latestCourse}</span>
