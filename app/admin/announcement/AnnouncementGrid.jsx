...
const AnnouncementGrid = ({ announcement, onDelete, setAnnouncementId, setIsAnnouncementOpen }) => {
...
//   (!announcement?.course || announcement?.course.length === 0 )&&
//   (!announcement?.department || announcement?.department.length === 0)&&
//   (!announcement?.location || announcement?.location.length === 0)
// console.log("isGlobalAnnouncement" , isGlobalAnnouncement)
//   const isUserMatched   =
//   (user?.course && course?.some(course => user.course.includes(course))) ||
//   (user?.department && department?.some(department => user.includes(department))) || 
//   (user?.location && location?.some(location => user.location.includes(location)))

//  if(!isGlobalAnnouncement && !isUserMatched){
//   return null
//  }
  ...
  return (
    <>
      <DeleteConfirmationDialog
        ...
        onConfirm={() => onDelete(announcement?.documentId)}
      />


      <Card className="hover:translate-y-1 transition-all relative courseGridBox">

        <CardHeader className="flex lg:flex-row flex-col flex-wrap justify-between gap-6 items-align mb-0">
           <div>
                          <Avatar className="rounded h-12 w-12">
                            <AvatarImage src={announcement.cardImage} alt={announcement.title || ""} />
                            <AvatarFallback className="rounded uppercase bg-success/30 text-success">
                              {announcement?.title?.slice(0, 2)}
                            </AvatarFallback>
                          </Avatar>
                        </div>
        ...
        <CardContent className="p-4 pt-0">
          {/* {announcement.cardImage && <div className="w-full max-h-[140px] bg-muted-foreground overflow-hidden rounded-md">
            <Image
          ...
          <div className="py-4 truncate-multiline">
