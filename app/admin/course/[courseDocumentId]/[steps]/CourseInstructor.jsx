...
function CourseInstructor({ handleBack, handleNext, course, isPublished }) {
    ...
    startTransition(async () => {
      try {
        ...
        await axiosInstance({
          ...
          // { "data": { ...formData?.data, course_enrollments: enrollments } }
        });
  ...
  const getAllInstructor = async (searchQuery = "") => {
    ...
    try {
      ...
      if (searchQuery) {
        queryParams.append(
          "filters[$or][0][firstName][$containsi]",
          searchQuery
        );
        queryParams.append(
          "filters[$or][1][lastName][$containsi]",
          searchQuery
        );
      ...
      setTimeout(() => {
        const instructors = course?.instructors?.map((ele) => ({
          label: `${ele.firstName} ${ele.lastName || ""}`,
          value: ele.id,
        }));
        reset({ instructors });
  ...
  useEffect(() => {
    const list = instructorsList.filter((ele) =>
      selectedInstructor?.find((ele1) => ele1.value === ele.id)
    );
        ...
        >
          <div className="p-0 bg-card rounded-md shadow-sm">
            <div className="col-span-12 mb-4 mt-0 space-y-1.5 px-6 py-6 mb-0 border-b border-border flex flex-row items-center">
              <h3 className="text-xl font-medium capitalize">Instructor</h3>
            </div>
