  ...
  return (
    <Card>
      ...
      <CardContent className="p-0 pt-0">
        ...
        {customers.length > 0 ? <div className="mt-0">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-6 lightOrange">
            {customers.slice(0, 3).map((customer, index) => (
              <CustomerCard key={customer.id}
                item={{
                  name: customer.user?.firstName || customer.user?.username, // Use firstName if available, else username
                  score: customer.point, // Assuming points map to the score
                  image: customer.user.profileImage?.url, // Profile image URL
                  color: "yellow", // Example color, adjust if needed
                  amount: customer.amount || 0, // Placeholder for amount
                }}
                index={index + 1} />
            ))}
          ...
          <div className="mt-8 p-6 pt-0">
            {customers.slice(3).map((customer, index) => (
              <ListItem key={`customer-${customer.id}`} item={{
                name: customer.user?.firstName || customer.user?.username, // Use firstName if available, else username
                score: customer.point, // Assuming points map to the score
                image: customer.user.profileImage?.url, // Profile image URL
                color: "yellow", // Example color, adjust if needed
        ...
        </div> : <p className="text-md text-slate-500 font-semibold courseHeading text-center p-3">No Leaderboard Available</p>}
      </CardContent>
    </Card>
  );
};
