...
const ShowAssessmentDetails = ({ setShowQuiz, startTimer, assignmentDetails, questions, setDocumentId, current_attempts, exitFullscreen }) => {
        ...
        if (current_attempts + 1 <= valid_attempts && instructions) {
            try {
                ...
                if (data?.data?.documentId) {
                    ...
                    // Add small buffer to avoid white screen during re-render
                    setTimeout(() => {
                      setShowQuiz(false);
                      startTimer();
                    }, 300);
    ...
    const getUserResultData = async () => {
        try {
            const { data } = await axiosInstance({
                url: `/api/attempt-contents?filters[user][id][$eq]=${user.id}&filters[assignment][documentId][$eq]=${params.id}&sort=publishedAt:desc&pagination[limit]=1&populate[certificate]=true`,
                method: "GET",
            });
            const result = data.data;
            console.log("getUserResultData", result);
            setCertificateInitLength(result.certificate)
        } catch (error) {
            console.log(error);
        }
    }
    ...
    return (
        ...
        <div class="">
            ...
            <div class="relative bg-card rounded-md shadow-sm p-8 w-full mx-auto mt-10 2xl:mt-20 max-w-[700px] mb-20">
                ...
                {/* <!-- Exam Details --> */}
                {current_attempts < valid_attempts ? (
                    <div className="grid grid-cols-2 gap-4 mb-8">
                        ...
                        <div className="flex flex-col border rounded-md p-2 text-center min-h-[80px] items-center justify-center">
                            ...
                            <p className="font-bold text-lg">{mcqQuestions} MCQ</p>
                            <p className="font-bold text-lg">{subjectiveQuestions} Subjective</p>
                        </div>
                        {/* Total Questions */}
                        <div className="flex flex-col border rounded-md p-2 text-center min-h-[80px] items-center justify-center">
                            <p className="text-md text-gray-500">Total Questions</p>
                            <p className="font-bold text-lg">{totalQuestions}</p>
                        </div>
                        {/* Total Marks */}
                        <div className="flex flex-col border rounded-md p-2 text-center min-h-[80px] items-center justify-center">
                            <p className="text-md text-gray-500">Total Marks</p>
                            <p className="font-bold text-lg">{max_score}</p>
                        </div>
                        {/* Passing Marks */}
                        <div className="flex flex-col border rounded-md p-2 text-center min-h-[80px] items-center justify-center">
                            <p className="text-md text-gray-500">Passing Marks</p>
                            <p className="font-bold text-lg">{min_score}</p>
                        </div>
                    </div>
                ...
                <div class="text-center">
                    ...
                    {current_attempts < valid_attempts ? (
                        ...
                        <p class="text-lg mt-4 mb-4"><span class="font-bold text-default-800">{current_attempts < valid_attempts ? current_attempts + 1 : current_attempts} / {valid_attempts} attempts are completed</span></p>
                    ...
                    <div className="flex flex-wrap gap-3 lg:gap-5 justify-center">
                        {current_attempts < valid_attempts ? (
                            <Button disabled={current_attempts === valid_attempts} color="primary" onClick={() => {
                                startQuiz()
                        ...
                        {certificateInitLength > 0 ? (
                            <div className="text-center flex justify-center gap-4">
                                <Button color="info" onClick={getTheUserResultData}>
                        ...
                        <Button color="dark" variant="outline" className="" onClick={() => {
                            backScreen()
                        }}
                        >
                            Back
