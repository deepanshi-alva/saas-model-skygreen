...
import { formatDateToIST, formatTime } from "@/lib/utils";
...
const Messages = ({
  ...
  const { user, answer: ans, createdTime: time, replayMetadata } = answer;
  ...
  return (
    <>
      <div className="block md:px-0 px-0 ">
        {/* {senderId === 11 ? (
          <>
            {replayMetadata === true && (
        ...
        <div className="flex space-x-2 items-start group rtl:space-x-reverse mb-4">
          <div className="flex-none self-end -translate-y-5">
            <div className="h-8 w-8 rounded-full">
              {/* <Image
                src={`${process.env.NEXT_PUBLIC_STRAPI_URL}${user.profileImage.url}`}
                // alt={avatar}
                className="block w-full h-full object-cover rounded-full"
                width={40}
                height={40}
              ...
              ? getFilePath(user.profileImage.url)
              ...
              <div className="flex items-center gap-1">
                <div className="whitespace-pre-wrap break-all relative z-[1]">
                  ...
                  <div className="text-base font-semibold">
                    {user?.username} <span className="text-default-500 text-sm font-normal">{formatDateToIST(time).date}</span>
                  </div>
                  <div
                    // className="text-base font-normal mt-0"
                    className="text-base py-2 px-0 flex-1  "
