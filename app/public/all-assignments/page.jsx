...
function Assignment() {
    ...
    const [meta, setMeta] = useState({ "page": 1, "pageSize": data?.pageSize || 12, "pageCount": 0, "total": 0, "currentPage": 1 });
    ...
    const columns = [
        ...
        },
        {
            ...
            header: ({ column }) => (
                <SortHeader column={column} title="Category" />
            ),
            cell: ({ row }) => {
                return (
                    <div className="flex gap-2">
                        {row?.original?.courses_categories && (
                            <span className="max-w-[500px] truncate font-medium">
                                {row?.original?.courses_categories.map(category => category?.title) || "-"}
                            </span>
                        )}
                    </div>
                );
                ...
                return <div className="flex justify-end">
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="">
                            <DropdownMenuItem onSelect={() => router.push(`/take-assessment/${row.original.documentId}`)}>Take Assesment</DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
                </div>
            },
    ...
    const fetchAllAssignments = async (pageNo = 1) => {
        try {
            ...
            const { data } = await axiosInstance.get(
                `/api/assignments?populate[course][populate][course_enrollments][populate]=user&populate[questions][populate]=*&populate[courses_categories][populate]=*&populate[certificate][populate]=*&sort=createdAt:desc&filters[available][$eq]=true&${query.toString()}&pagination[page]=${pageNo}&pagination[pageSize]=${pageSize}`
            );
            ...
            tempAssignments = await Promise.all(
                data.data.map(async (ele) => {
                    ...
                    try {
                        ...
                        console.error(`Error fetching attempts for documentId: ${ele.documentId}`, error);
                    }
    ...
    return (
        ...
        <>
            {(!assignments.length && !filteredAssignments.length && !filterBy.length) ?
                (<>
            ...
            {filteredAssignments.length === 0 && (searchQuery || filterBy.length > 0) ? (
                ...
                <>
                    ...
                    {pageView === "list" && (
                        <ProjectList data={assignments} table={table} columns={columns} meta={meta} func={fetchAllAssignments} />
                    )}
                </>
            )}
        </>
